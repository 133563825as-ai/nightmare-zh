# Nightmare Mobile 汉化版

> **简体中文汉化版 · 第三方非官方改编**
>
> 本项目是 [AbrahamPaulJ/nightmare-mobile](https://github.com/AbrahamPaulJ/nightmare-mobile)
> 的**简体中文本地化版本**，基于上游 **v1.6.0**（`versionCode=306`）。
>
> - **改动内容**：仅界面文案汉化 —— 新增 `values-zh` 翻译（574 条），并把界面中残留的
>   英文字面量接入字符串资源。**未改动** NPU 推理、模型下载、图像处理等任何逻辑，
>   出图结果与上游完全一致。
> - **许可证**：沿用上游 **CC BY-NC 4.0**（署名—非商业性使用 4.0 国际）。
>   **禁止商业用途**。转载或再改编请保留署名并注明改动。
> - **署名**：原作者 **Abraham Paul Jaison**，以及 `NOTICE` 中列出的全部第三方组件
>   （xororz/local-dream、Qualcomm QNN、QuickJS、MNN、Neodragon 等）。
> - **本仓库不含二进制**：APK 与高通 QNN 运行时作为 **Release 附件**发布，
>   与上游做法一致（上游 `.gitignore` 同样排除 `jniLibs/`、`assets/qnnlibs/`、
>   `assets/ditlibs/`）。
>
> ⚠️ **安装前请注意**：本汉化版与上游官方版**签名不同**，无法互相覆盖安装。
> 若手机已装官方版，需先卸载（**已下载的模型会被清除**）。
