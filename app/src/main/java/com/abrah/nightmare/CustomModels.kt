...
object CustomModels {
    ...
    fun importDit(
        ...
        // ⚠ 1.6.0 把这段 `when` 收进了 [markOf]；译文跟着搬进去（见该方法）。
        ...
        onProgress(
            ModelInstaller.Progress(
                NmApp.str(R.string.phase_checking_file, "checking the file"), 0, 0,
            ),
        )
        ...
        try {
            ...
            if (dest.length() <= 0L) {
                throw java.io.IOException(
                    NmApp.str(R.string.err_picked_file_empty, "the picked file was empty"),
                )
            }
    ...
    private fun ensureDitParts(
        ...
        for (step in plan.sortedBy { it.from == null }) {
            ...
            if (step.from != null) {
                onProgress(
                    ModelInstaller.Progress(
                        NmApp.str(R.string.phase_preparing_shared, "preparing the shared parts"),
                        before, total,
                    ),
                )
                ...
                ModelInstaller.fetch(
                    f.url, step.dest, f.bytes,
                    NmApp.str(R.string.phase_downloading_shared, "downloading the shared parts"), { p ->
    ...
    private fun markOf(family: Family): String = when (family) {
        ...
        Family.FLUX2 -> FLUX2_MARK
            ...
            NmApp.str(
                R.string.err_family_no_safetensors,
                "%1\$s does not import a plain .safetensors",
                family.toString(),
            ),
        )
    }
    ...
    private fun checkSafetensors(open: () -> InputStream) {
        ...
        open().use { input ->
            if (input.readNBytes(head, 0, 8) != 8) {
                throw java.io.IOException(
                    NmApp.str(R.string.err_file_too_small, "this file is too small to be a checkpoint"),
                )
            }
            var len = 0L
    ...
    fun import(
        ...
        if (spec == null) {
            ...
            throw IOException(
                NmApp.str(
                    R.string.err_not_a_checkpoint,
                    "not a checkpoint: no %1\$s (SD 1.5), %2\$s (SDXL) or %3\$s (Anima) in the archive",
                    SD15_MARK, SDXL_MARK, ANIMA_MARK,
                ),
            )
        }
        scan(context)
        ...
        return files.map { file ->
            ...
            try {
                val spec = if (dit != null) {
                    ...
                    throw IOException(
                        NmApp.str(
                            R.string.err_dit_family_in_name,
                            "a .safetensors needs its family in the name — %1\$s",
