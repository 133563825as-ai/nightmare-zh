    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        ...
        for (port in spec.inputs) {
            val v = inputs[port.name]
                ?: throw IllegalArgumentException(
                    NmApp.str(R.string.err_input_not_connected, node.id, port.name)
                )
        ...
        val res = try {
            ...
            throw IllegalStateException(
                NmApp.str(
                    R.string.err_not_an_object,
                    "node \"${node.id}\" returned %2\$s, which is not an object",
                    node.id,
                    out.take(80),
                )
            )
        ...
        val port = spec.outputs.firstOrNull()
            ?: throw IllegalStateException(
            NmApp.str(
                R.string.err_no_outputs,
                "node type %1\$s declares no outputs",
                name,
            ),
        )
        ...
        return when (port.type) {
            "IMAGE" -> {
                val bmp = ctx.images.get(id)
                    ?: throw IllegalStateException(
                        NmApp.str(
                            R.string.err_image_handle_dangling,
            ...
            "LATENT" -> Value.Handle(id, "latent")
                ...
                NmApp.str(
                    R.string.err_unknown_output_type,
                    "node type %1\$s declares output type \"%2\$s\", which this app " +
    ...
    private fun requirePortType(node: Node, port: Port, v: Value) {
        val ok = when (port.type) {
            ...
            "LATENT" -> v is Value.Handle && v.kind == "latent"
                ...
                NmApp.str(
                    R.string.err_unknown_input_type,
                    "node type %1\$s declares input \"%2\$s\" as \"%3\$s\", " +
        ...
        if (!ok) {
            throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_port_type_mismatch,
