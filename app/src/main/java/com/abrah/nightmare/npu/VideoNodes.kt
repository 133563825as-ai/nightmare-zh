...
object VideoSampleNode : NodeType {
    ...
    override fun titleFor(node: com.abrah.nightmare.Node): String =
        if (node.inputs["image"] != null) NmApp.str(R.string.recipe_i2v_label, "Image to video")
        else NmApp.str(R.string.recipe_t2v_label, "Text to video")

    override val defaultId = "video"
    override val about = NmApp.str(
        R.string.palette_about_video,
        "a prompt, or a prompt and a photo, into a 2 second clip",
    )
    ...
    override val widgets get() = listOf(
        ...
        Widget(
            "seed", "int", "0",
            hint = NmApp.str(R.string.hint_video_seed, "0 = a new clip every Run. Type the seed shown on the node to get it back."),
        ),
        Widget(
            "upscale", "bool", "true",
            // ⚠ 1024x640 / 512x320 — width first, which is the way round the
            // frames actually come out. Neodragon's docs use (h, w).
            hint = NmApp.str(R.string.hint_quicksr, "2x to 1024x640 with QuickSRNet. Off renders 512x320 and is a little faster."),
        ),
        ...
        Widget(
            "x", "float", "0.0", 0.0, 1.0,
            hint = NmApp.str(R.string.hint_drag_frame, "drag the frame on the picture"),
        ),
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value =
        ...
        val android = ctx.android
            ?: throw IllegalStateException(
                NmApp.str(
                    R.string.err_video_needs_context,
        ...
        if (missing.isNotEmpty()) {
            throw IllegalStateException(
                NmApp.str(
                    R.string.err_video_models_missing_path,
                    "node \"${node.id}\": %2\$d video model(s) not on this device " +
                        "(\"%3\$s\") — they live in %4\$s",
                    node.id,
                    missing.size,
                    missing.take(3).joinToString() +
                        if (missing.size > 3) ", …" else "",
                    NpuFiles.ctxDir(android),
                )
            )
        }
        ...
        if (missingAssets.isNotEmpty()) {
            throw IllegalStateException(
                NmApp.str(
                    R.string.err_video_weights_missing,
                    "node \"${node.id}\": video weights missing: %2\$s",
                    node.id,
        ...
        val bitmap: Bitmap? = source?.let { img ->
            val src = ctx.images.get(img.id)
                ?: throw IllegalStateException(
                    NmApp.str(R.string.err_img_evicted, node.id, img.id)
                )
        ...
        try {
            ...
            ctx.say(
            NmApp.str(
                if (bitmap != null) R.string.say_seed_animating else R.string.say_seed,
                "seed %1\$s",
                used,
            ),
        )
            ...
            ctx.say(
            NmApp.str(
                R.string.say_frames_encoding,
                "%1\$d frames in %2\$s s — encoding",
                frames.size,
                "%.1f".format(r.seconds),
            ),
        )
...
object VideoOutputNode : NodeType {
    ...
    override val widgets = listOf(
        Widget(
            "save", "bool", "true",
            hint = NmApp.str(
                R.string.hint_write_mp4,
                "write an MP4 to Movies/Nightmare, which outlives the app's cache",
            ),
        ),
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value =
        ...
        val video = inputs["video"] as? Value.Video
            ?: throw IllegalArgumentException(
                NmApp.str(R.string.err_input_not_connected, node.id, "video")
            )
        ...
        check(file.isFile) {
            NmApp.str(R.string.err_clip_gone, node.id, file.parent ?: "")
        }
