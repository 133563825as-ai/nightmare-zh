...
abstract class VideoNode : NodeType {
    ...
    protected fun android(ctx: NodeCtx, node: Node): android.content.Context =
        ctx.android ?: throw IllegalStateException(
            NmApp.str(R.string.err_video_needs_context, node.id)
        )
    ...
    protected fun requireModels(ctx: NodeCtx, node: Node, needed: List<String>) {
        ...
        if (missing.isNotEmpty()) {
            throw IllegalStateException(
                NmApp.str(
                    R.string.err_video_models_missing,
                    "node \"${node.id}\": %2\$d video model(s) not on this device " +
                        "(%3\$s) — install them under Models > Video",
                    node.id,
                    missing.size,
...
object VideoClipEncodeNode : VideoNode() {
    ...
    override val widgets = listOf(
        Widget(
            ...
            hint = NmApp.str(R.string.hint_what_to_animate, "what to animate"),
        ),
    )
    ...
    override suspend fun runPorts(
        ...
        ctx.say(NmApp.str(R.string.say_encoding_prompt, "encoding the prompt"))
        ...
        if (wantsFrame) {
            ctx.say(NmApp.str(R.string.say_encoding_prompt_first, "encoding the prompt for the first frame"))
        ...
        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            ...
            val h = inputs["cond"] as? Value.Tensors
                ?: throw IllegalArgumentException(
                    NmApp.str(R.string.err_input_not_connected, node.id, "cond")
                )
            val b = TensorStore.get(h.id)
                ?: throw IllegalStateException(
                    NmApp.str(
                        R.string.err_conditioning_evicted,
                        NmApp.str(R.string.err_conditioning_evicted, node.id),
                    )
                )
            val seed = node.params["seed"]?.toLongOrNull() ?: 0L
            val used = if (seed == 0L) System.currentTimeMillis() else seed
            ctx.say(NmApp.str(R.string.say_first_frame_seed, "first frame, seed %1\$s", used))
...
object VideoVaeEncodeNode : VideoNode() {
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value =
        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            ...
            ctx.say(NmApp.str(R.string.say_encoding_first_frame, "encoding the first frame"))
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value =
        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            ...
            val lh = inputs["latent"] as? Value.Tensors
                ?: throw IllegalArgumentException(
                    NmApp.str(R.string.err_input_not_connected, node.id, "latent")
                )
            val cb = TensorStore.get(ch.id) ?: throw IllegalStateException(
                NmApp.str(R.string.err_conditioning_evicted, node.id)
            )
            val lb = TensorStore.get(lh.id) ?: throw IllegalStateException(
                NmApp.str(R.string.err_latent_evicted_run_again, node.id)
            )
            val seed = node.params["seed"]?.toLongOrNull() ?: 0L
            val used = if (seed == 0L) System.currentTimeMillis() else seed
            val r = runner(ctx, node)
            ...
            ctx.say(NmApp.str(R.string.say_seed, "seed %1\$s", used))
...
object VideoVaeDecodeNode : VideoNode() {
    ...
    override val widgets = listOf(
        Widget(
            "upscale", "bool", "true",
            hint = NmApp.str(
                R.string.hint_quicksr,
                "2x to 1024x640 with QuickSRNet. Off renders 512x320 and is a little faster.",
            ),
        ),
    )
