...
class HostSurface(
    ...
    override fun call(op: String, argsJson: String): String {
        ...
        return when (op) {
            ...
            "latent.blend" -> {
                val ops = latents
                    ?: throw IllegalStateException(
                        NmApp.str(
                            R.string.err_blend_needs_backend,
                ...
                val png = images.png(maskId)
                    ?: throw IllegalArgumentException(
                        NmApp.str(
                            R.string.err_unknown_mask_handle,
            ...
            // that gets `null` back would fail somewhere else entirely.
                ...
                NmApp.str(R.string.err_unknown_op, "unknown host op \"%1\$s\"", op),
            )
        }
    }
    ...
    private fun color(spec: String): Int = try {
        ...
        throw IllegalArgumentException(
            NmApp.str(
                R.string.err_not_a_colour,
                "not a colour: \"%1\$s\" (use #rrggbb or #aarrggbb)",
                spec,
            ),
        )
    }
    ...
    private fun image(args: JSONObject, field: String): Bitmap {
        ...
        return images.get(id)
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_unknown_image_handle,
