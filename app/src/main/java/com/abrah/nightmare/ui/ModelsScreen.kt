...
fun ModelsScreen(
        ...
        AlertDialog(
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    if (p.unsavedFlow) {
                        Text(
                            stringResource(R.string.flow_unsaved_open_model),
                            style = LogTextStyle,
                    ...
                    for (r in p.offer) {
                        Surface(
                            ...
                            Column(Modifier.padding(horizontal = 14.dp, vertical = 10.dp)) {
                                Text(r.labelText, fontWeight = FontWeight.Medium)
                                Text(
                                    r.aboutText,
                                    style = LogTextStyle,
                    ...
                    if (p.spec != null) Surface(
                        ...
                        Text(
                            stringResource(R.string.keep_canvas_flow),
        ...
        SwipeTabs(
            ...
            LazyColumn(
                ...
                item {
                    Text(
                        when (family) {
                            Family.SD15 -> stringResource(R.string.family_note_sd15)
                            ...
                            Family.ANIMA -> stringResource(R.string.family_note_anima)
                            // ⚠ Plain files straight into place, so no unpack
                            // headroom — but only an 8 Elite or newer runs them.
                            Family.FLUX2 -> stringResource(R.string.family_note_flux2)
                            ...
                            Family.ZIMAGE -> stringResource(R.string.family_note_zimage)
                            ...
                            else -> stringResource(R.string.family_note_sdxl)
                        },
                        style = LogTextStyle,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
    ...
    deletingUpscaler?.let { spec ->
        ...
        ConfirmDelete(
            title = stringResource(R.string.delete_model, spec.label),
            body = stringResource(
                R.string.delete_upscaler_body,
                mb(row?.onDisk ?: 0L).toInt(),
                mb(row?.build?.bytes ?: 0L).toInt(),
            ),
    ...
    if (deletingSegmenter && segmenter != null) {
        ConfirmDelete(
            title = stringResource(R.string.delete_model, segmenter.label),
            body = stringResource(
                R.string.delete_segmenter_body,
                mb(segmenter.onDisk).toInt(),
                mb(segmenter.bytes).toInt(),
            ),
    ...
    deletingEmbedding?.let { name ->
        ConfirmDelete(
            title = stringResource(R.string.delete_embedding, name),
    ...
    deleting?.let { spec ->
        ...
        ConfirmDelete(
            title = stringResource(R.string.delete_model, spec.label),
            body = buildString {
                append(stringResource(R.string.delete_model_body, mb(row?.onDisk ?: 0L).toInt()))
                ...
                if (spec.isCustom || bytes == null) {
                    append(stringResource(R.string.delete_model_imported_body))
                } else {
                    append(
                        stringResource(R.string.delete_model_download_body, mb(bytes).toInt())
                    )
                }
                ...
                if (row?.selected == true) {
                    append("\n\n")
                    append(stringResource(R.string.delete_model_in_use_note))
                }
            },
...
private fun ImportCard(
    ...
    ImportCallout(
        title = stringResource(R.string.import_checkpoint),
        body = if (dit) {
            ...
            // ⭐⭐ **1.6.0 起没有前置条件了**，文案跟着改（原文在 2026-09-21 前写的是
            // 「先下载内置模型」，那是 `ensureDitParts` 当年抛出的拒绝 —— 为了拿
            // 2.6 GB 而索要 6.7 GB）。现在零件按需取，所以剩下要说的只有体积，一次说完。
            // ⚠ 三个数字而不是一个：零件不同行。文本编码器和分词器（2.27 GB）两个
            // DiT 族逐字节相同，VAE（336 MB）不同，所以「有另一个族」能覆盖大部分但
            // 不是全部。`CustomModels.DIT_FAMILY_AGNOSTIC` 里是 md5。
            stringResource(R.string.import_dit_body)
        } else {
            ...
            stringResource(R.string.import_zip_body)
        },
        enabled = !busy,
    ...
    if (naming) {
        ...
        AlertDialog(
            ...
            title = { Text(stringResource(R.string.name_it)) },
            text = {
                Column {
                    OutlinedTextField(
                        value = name,
                    ...
                    Text(
                        when {
                            reserved -> stringResource(R.string.name_reserved)
                            trimmed.isNotEmpty() && !ok -> stringResource(R.string.name_bad_chars)
                            ...
                            trimmed.isEmpty() -> stringResource(R.string.name_empty_hint)
                            else -> stringResource(R.string.name_used_hint)
                        },
                        style = LogTextStyle,
                        color = if (reserved) {
            ...
            confirmButton = {
                Button(
                    ...
                    enabled = ok,
                ) { Text(stringResource(R.string.pick_a_zip)) }
            },
...
private fun UpscalerCard(
    ...
    DownloadCard(
        ...
        detail = fields(row.spec.aboutText, row.build?.tier),
        progress = row.progress,
    ) {
        when {
            row.progress != null ->
