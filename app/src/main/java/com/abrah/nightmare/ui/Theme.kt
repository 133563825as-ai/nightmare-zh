...
/**
 * ⭐ Measurement text — byte counts, step counts, seeds, model sizes.
 *
 * ⚠⚠⚠ **This was `FontFamily.Monospace`, and that is what made the Chinese
 * build stutter.** Monospace resolves to the platform's monospaced LGC family,
 * which carries NO CJK glyphs, so every Chinese character missed the primary
 * family and went through the font-fallback chain instead. Upstream never paid
 * it: every string in this style was English, so the primary family always hit.
 *
 * ⚠ Reported from the phone 2026-09-21 as "滑动一顿一顿，往下滑的时候也有点卡"
 * on the Models tab — ~17 cards, each several lines of Chinese in this style,
 * which is exactly the shape a per-character fallback lookup takes.
 *
 * ⚠⚠ The cost is `FontFamily.Default`, not nothing: this style loses its fixed
 * advance width, so a column of numbers no longer lines up as it did. That is
 * the right trade — a readable, smooth list beats an aligned one — and the
 * amount of truly tabular text in this app is small. ⚠ If alignment is ever
 * needed again, the answer is `FontFamily.Monospace` on the LATIN runs only
 * (`AnnotatedString` with a span), never on a mixed string.
 */
