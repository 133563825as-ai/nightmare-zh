...
fun DeviceSheet(caps: DeviceProbe.Caps, onDismiss: () -> Unit) {
    AlertDialog(
        ...
        title = { Text(stringResource(R.string.this_device)) },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(androidx.compose.ui.unit.Dp(2f))) {
                Field(stringResource(R.string.field_chip), caps.soc.ifBlank { "unknown" })
                Field(stringResource(R.string.field_htp_arch), "v${caps.arch}")
                Field(stringResource(R.string.field_vtcm), "${caps.vtcmMb} MB")
                Field(
                    ...
                    stringResource(
                        if (caps.measured) R.string.source_measured else R.string.source_assumed
                    ),
                )
                Spacer()
                ...
                if (!caps.staged) {
                    Text(
                        stringResource(R.string.device_no_qnn, "${caps.arch}") + " " +
                            stringResource(R.string.device_no_qnn_why) + " " +
                ...
                Field(
                    stringResource(R.string.field_models_runnable),
                    "$runnable of ${ModelCatalog.builtIn.size}",
                )
                val tiers = ModelCatalog.sd15Models.firstOrNull()?.buildFor(caps)?.tier
                if (tiers != null) {
                    Field(stringResource(R.string.field_sd15_build), tiers.removePrefix("_"))
                }
                Field(
                    "SDXL",
                    if (ModelCatalog.sdxlModels.first().buildFor(caps) != null) {
                        ...
                        stringResource(R.string.sdxl_needs_v75)
                    },
                )
                ...
                if (!caps.measured) {
                    Spacer()
                    Text(
                        stringResource(R.string.device_measured_later),
                        style = LogTextStyle,
