...
enum class LibraryTab(val label: String) {
    ...
    /**
     * ⭐⭐ The tab's name in the phone's language.
     *
     * ⚠ An accessor rather than a translated [label]: an enum constant is
     * initialized with its class, before an Application exists, so it cannot
     * read a resource. ⚠ The `nav_*` keys already existed in `values-zh` —
     * upstream translated them and then never referenced them anywhere, which
     * is why the tabs read "Models/Flows/Results" over a Chinese UI.
     */
    val labelText: String
        get() = com.abrah.nightmare.NmApp.str(
            when (this) {
                MODELS -> com.abrah.nightmare.R.string.nav_models
                FLOWS -> com.abrah.nightmare.R.string.nav_flows
                RESULTS -> com.abrah.nightmare.R.string.nav_results
            },
            label,
        )
}
    ...
    Row(
        ...
        androidx.compose.material3.IconButton(onClick = onClose) {
            androidx.compose.material3.Icon(
                ...
                contentDescription = stringResource(R.string.close_back_to_canvas_plain),
