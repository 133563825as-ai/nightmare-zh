...
object PromptNode : NodeType {
    ...
    override val widgets get() = listOf(
        Widget(
            ...
            hint = NmApp.str(R.string.hint_what_to_draw, "what to draw"),
        ),
        Widget(
            "negative", "string", SelectedModel.spec.starterNegative,
            hint = NmApp.str(R.string.hint_keep_out, "what to keep out of the picture"),
        ),
    )
...
class SdSampler(
     ...
     */
    override fun titleFor(node: Node): String =
        NmApp.str(familyLabelRes(family), family.label) + " " + jobFor(node)
    ...
    fun jobFor(node: Node): String = when {
        inpaint -> NmApp.str(R.string.sampler_kind_inpaint, "Inpaint")
        family == Family.FLUX2 &&
            (node.inputs["image"] != null || node.inputs["reference"] != null) ->
            NmApp.str(R.string.sampler_kind_edit, EDIT_LABEL)
        node.inputs["image"] != null -> NmApp.str(R.string.sampler_kind_img2img, "Image to image")
        else -> NmApp.str(R.string.sampler_kind_txt2img, "Text to image")
    }

    override val defaultId = family.slug + if (inpaint) "_inpaint" else "_generate"

    override val about = if (inpaint) {
        NmApp.str(R.string.hint_paint_area_only, "paint an area of a photo and re-imagine only that")
    } else {
        NmApp.str(
            R.string.palette_about_image,
    ...
    private fun baseWidgets() = listOf(
        ...
        Widget(
            "seed", "int", "0",
            hint = NmApp.str(
                R.string.hint_seed_zero,
                "0 = a new picture every Run. Type the seed shown on the node to get that one back.",
            ),
        ),
        ...
        Widget(
            ...
            hint = NmApp.str(
                R.string.hint_cfg_distilled,
                "the sampler; a distilled model usually needs the one its author published",
            ),
        ),
        ...
        Widget(
            ...
            options = REF_SIZES,
            hint = NmApp.str(
                R.string.hint_reference_size,
                "the longest edge the reference is sent at, its own aspect kept. " +
        ...
        Widget(
            ...
            hint = NmApp.str(R.string.hint_outpaint_fill, "what fills the frame where it runs off the photo"),
        ),
        ...
        *(if (!inpaint) emptyArray() else arrayOf(
            Widget(MaskNode.OPS, "string", "", hint = NmApp.str(R.string.hint_paint_area_redo, "paint the area to redo")),
            ...
            Widget(
                ...
                hint = NmApp.str(R.string.hint_crop_around_mask, "render a crop around the mask: more detail where you painted"),
            ),
            Widget(
                PasteNode.STITCH, "bool", "false",
                hint = NmApp.str(R.string.hint_paste_back, "off: the result is the frame you chose. on: it is pasted back into the whole photo"),
            ),
        )),
    ...
    private fun ditWidgets(): List<Widget> = baseWidgets()
        ...
        Widget(
            ...
            hint = NmApp.str(
                R.string.hint_cfg_distilled,
                "1 is what these models are distilled for. Above 1 the negative prompt " +
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        ...
        val prompt = inputs["prompt"] as? Value.Prompt
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_nothing_wired_prompt,
        ...
        suspend fun cond(): String = condCache ?: run {
            ctx.say(NmApp.str(R.string.say_reading_prompt, "reading the prompt"))
        ...
        val src = ctx.images.get(photo.id)
            ?: throw IllegalStateException(
                NmApp.str(
                    R.string.err_img_evicted,
                    "node \"${node.id}\": image %2\$s is no longer in the store",
                    node.id, photo.id,
                )
            )
        ...
        val painted = if (!MaskTaps.hasTaps(stored)) stored else {
            ...
            if (android == null || !com.abrah.nightmare.segment.Segmenter.isInstalled(android)) {
                throw IllegalStateException(
                    NmApp.str(
                        R.string.err_tapped_mask_needs_segmenter,
                        "node \"${node.id}\": this mask was tapped — download " +
                            "${com.abrah.nightmare.segment.Segmenter.LABEL} in Models, Tools",
                    )
                )
            }
            ctx.say(NmApp.str(R.string.say_finding_objects, "finding the objects you tapped"))
        ...
        if (!masking) {
            ctx.say(NmApp.str(R.string.say_reimagining, "re-imagining the picture"))
        ...
        ctx.say(
            if (painted.isEmpty) NmApp.str(R.string.say_filling_padding, "filling the padding")
            else NmApp.str(R.string.say_repainting_marked, "repainting the area you marked"),
        )
        ...
        val patchBmp = ctx.images.get(patch.id)
            ?: throw IllegalStateException(
                NmApp.str(
                    R.string.err_render_vanished,
                    "node \"${node.id}\": the render vanished from the store",
                ),
            )
        return finishInpaint(ctx, node, p, src, frame, frameRect, cut, patchBmp)
    }
    ...
    private suspend fun runDitMasked(
        ...
        ctx.say(NmApp.str(R.string.say_repainting_marked, "repainting the area you marked"))
        ...
        return android.graphics.BitmapFactory.decodeByteArray(out.png, 0, out.png.size)
            ?: throw IllegalStateException(
                NmApp.str(
                    R.string.err_engine_decode,
                    "node \"${node.id}\": the engine's picture would not decode",
                ),
            )
    }
    ...
    private suspend fun runDit(
        ...
        val referencePng = reference?.let {
            val bmp = ctx.images.get(it.id)
                ?: throw IllegalStateException(
                    NmApp.str(
                    R.string.err_ref_img_evicted,
                    "node \"${node.id}\": reference image %2\$s is no longer in the store",
                    node.id, it.id,
                )
                )
...
object MediaOutputNode : NodeType {
    ...
    override val widgets = listOf(
        ...
        Widget(
            AUTOSAVE, "bool", "true",
            hint = NmApp.str(R.string.hint_keep_every_run, "keep every Run in Results"),
        ),
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        val media = inputs["media"]
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_nothing_wired_media,
        ...
        if (media !is Value.Image && media !is Value.Video) {
            throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_not_picture_or_clip,
...
/**
 * ⭐⭐ **A family's display name in the phone's language.**
 *
 * ⚠⚠ `Family.label` is an ENUM CONSTRUCTOR argument, so it is fixed at class
 * load — before an Application exists — and a resource read there would cache
 * the English fallback for the process ([NmApp.str]'s own note). Every surface
 * that SHOWS the family asks this instead.
 *
 * ⚠ The values are the marketing names of model families, and are kept
 * recognisable: "SD 1.5" stays "SD 1.5" because that is what a checkpoint file
 * and every Civitai page call it. Only the words around them are translated.
 */
fun familyLabelRes(f: com.abrah.nightmare.Family): Int = when (f) {
    com.abrah.nightmare.Family.SD15 -> R.string.family_sd15
    com.abrah.nightmare.Family.SDXL -> R.string.family_sdxl
    com.abrah.nightmare.Family.ANIMA -> R.string.family_anima
    com.abrah.nightmare.Family.FLUX2 -> R.string.family_flux2
    com.abrah.nightmare.Family.ZIMAGE -> R.string.family_zimage
}

/** ⭐ [familyLabelRes], resolved. */
fun familyLabelText(f: com.abrah.nightmare.Family): String =
    NmApp.str(familyLabelRes(f), f.label)
