...
internal fun Node.str(name: String): String =
    params[name] ?: throw IllegalArgumentException(
        NmApp.str(
            R.string.err_missing_param,
            "node \"%1\$s\": missing param \"%2\$s\"",
            id, name,
        ),
    )
...
internal fun Node.int(name: String): Int =
    str(name).toIntOrNull()
        ?: throw IllegalArgumentException(
            NmApp.str(
                R.string.err_param_not_int,
                "node \"%1\$s\": param \"%2\$s\" is not an int: %3\$s",
                id, name, params[name].toString(),
            ),
        )
...
private fun Node.dbl(name: String): Double =
    str(name).toDoubleOrNull()
        ?: throw IllegalArgumentException(
            NmApp.str(
                R.string.err_param_not_number,
                "node \"%1\$s\": param \"%2\$s\" is not a number: %3\$s",
                id, name, params[name].toString(),
            ),
        )
...
internal fun aspectWidget(spec: ModelSpec = SelectedModel.spec): Array<Widget> =
    if (!spec.fixedCanvas) emptyArray()
        ...
        Widget(
            ...
            hint = NmApp.str(
                R.string.hint_fixed_shape,
                "this family renders a fixed ${spec.native} and crops to shape — " +
...
object SampleNode : NodeType {
    ...
    override val widgets get() = listOf(
        ...
        Widget(
            "seed", "int", "0",
            hint = NmApp.str(R.string.hint_seed_zero, "0 = a new picture every Run. Type the seed shown on the node to get that one back."),
        ),
        ...
        Widget(
            ...
            hint = NmApp.str(R.string.hint_cfg_distilled, "the sampler; a distilled model usually needs the one its author published"),
        ),
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        ...
        val cond = inputs["cond"]
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_nothing_wired_cond,
...
object VaeDecodeNode : NodeType {
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        val latent = inputs["latent"]
            ?: throw IllegalArgumentException(NmApp.str(R.string.err_input_not_connected, node.id, "latent"))
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        ...
        val platform = ctx.android
            ?: throw IllegalStateException(
                NmApp.str(R.string.err_load_needs_context, node.id)
            )
        ...
        val bytes = try {
            if (uri.startsWith("content://")) {
                platform.contentResolver.openInputStream(android.net.Uri.parse(uri))
                    ?.use { it.readBytes() }
                    ?: throw IllegalArgumentException(
                        NmApp.str(R.string.err_nothing_at_uri, "nothing to read at %1\$s", uri),
                    )
            } else {
            ...
            throw IllegalStateException(
                NmApp.str(
                    R.string.err_read_permission,
                    "node \"${node.id}\": no longer permitted to read %2\$s — pick the image again",
                    node.id, uri,
                )
            )
        }
        ...
        val decoded = ctx.images.decode(bytes, MAX_EDGE)
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_undecodable,
                    "node \"${node.id}\": %2\$s is not an image this device can decode",
                    node.id, uri,
                )
            )
...
object TextEncodeNode : NodeType {
    ...
    override val widgets get() = listOf(
        Widget(
            ...
            hint = NmApp.str(R.string.hint_what_to_draw_wire, "what to draw — the sampler reads it through the cond wire"),
        ),
        Widget(
...
object VaeEncodeNode : NodeType {
    ...
    override val widgets get() = listOf(
        ...
        Widget(
            "seed", "int", "42",
            hint = NmApp.str(R.string.hint_noise_seed, "encoding noise, not the picture seed — it changes the result only slightly, so leave it fixed. The sample node's seed is the one that matters."),
        ),
    )
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        val image = inputs["image"] as? Value.Image
            ?: throw IllegalArgumentException(
                NmApp.str(R.string.err_input_not_connected, node.id, "image")
            )
        val png = ctx.images.png(image.id)
            ?: throw IllegalStateException(
                NmApp.str(R.string.err_img_evicted, node.id, image.id)
            )
...
object OutputNode : NodeType {
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        ...
        val android = ctx.android
            ?: throw IllegalStateException(
                NmApp.str(R.string.err_saving_needs_context, node.id)
            )
...
object MaskNode : NodeType {
    ...
    override val widgets = listOf(
        ...
        Widget(
            "grow", "float", "0.0", 0.0, 0.2,
            hint = NmApp.str(R.string.hint_spread_mask, "spread the mask outward"),
        ),
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        val src = inputs["image"] as? Value.Image
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_mask_needs_picture,
...
object MaskCropNode : NodeType {
    ...
    override val widgets = listOf(
        Widget(
            ...
            hint = NmApp.str(
                R.string.hint_crop_around_mask,
                "Generate over a crop around the mask: more detail where you painted, " +
    ...
    override suspend fun runPorts(
        ...
        val image = inputs["image"] as? Value.Image
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_image_not_connected_repaint,
        ...
        val mask = inputs["mask"] as? Value.Image
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_mask_not_connected,
                    "node \"${node.id}\": \"mask\" is not connected — wire the Mask node",
                )
            )
        val src = ctx.images.get(image.id)
            ?: throw IllegalStateException(NmApp.str(R.string.err_img_evicted, node.id, image.id))
        val maskBmp = ctx.images.get(mask.id)
            ?: throw IllegalStateException(NmApp.str(R.string.err_mask_evicted, node.id, mask.id))
...
object PasteNode : NodeType {
    ...
    override val widgets = listOf(
        Widget(
            STITCH, "bool", "false",
            hint = NmApp.str(
                R.string.hint_paste_back,
                "Off: the result is the frame you chose. On: it is pasted back into the " +
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        fun need(port: String, why: String): Value.Image =
            inputs[port] as? Value.Image
                ?: throw IllegalArgumentException(
                    NmApp.str(
                        R.string.err_port_not_connected_why,
                        "node \"${node.id}\": \"$port\" is not connected — %3\$s",
                        node.id, port, why,
                    ),
                )
...
object LatentBlendNode : NodeType {
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        ...
        val a = inputs["base"] as? Value.Handle
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_base_not_connected,
        ...
        val b = inputs["repaint"] as? Value.Handle
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_repaint_not_connected,
        ...
        val mask = inputs["mask"] as? Value.Image
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_decoder_mask_not_connected,
        ...
        val png = ctx.images.png(mask.id)
            ?: throw IllegalStateException(
                NmApp.str(R.string.err_mask_evicted, node.id, mask.id)
            )
...
object UpscaleNode : NodeType {
    ...
    override val widgets get() = listOf(
        ...
        Widget(
            ...
            hint = NmApp.str(R.string.hint_upscaler_pick, "which upscaler weights to use — install them under Models"),
        ),
    )
    ...
    override suspend fun run(ctx: NodeCtx, node: Node, inputs: Map<String, Value>): Value {
        val image = inputs["image"] as? Value.Image
            ?: throw IllegalArgumentException(
                NmApp.str(R.string.err_port_not_connected, node.id, "image")
            )
        val src = ctx.images.get(image.id)
            ?: throw IllegalStateException(
                NmApp.str(R.string.err_img_evicted, node.id, image.id)
        ...
        val android = ctx.android
            ?: throw IllegalStateException(
                NmApp.str(
                    R.string.err_upscale_needs_context,
        ...
        val path = UpscalerCatalog.pathFor(android, id)
            ?: throw IllegalArgumentException(
                NmApp.str(
                    R.string.err_upscaler_not_installed,
                    "node \"${node.id}\": upscaler \"%2\$s\" is not installed — open Models and download it",
                    node.id,
