...
object DitEngine {
    ...
    fun install(
        ...
        if (got != SHA256) {
            ...
            throw IOException(
                NmApp.str(
                    R.string.err_sha_mismatch,
                    "%1\$s: archive sha256 is %2\$s, expected %3\$s — the download is corrupt",
                    LABEL, got, SHA256,
                ),
            )
        }
        ...
        if (!extracted) {
            throw IOException(
                NmApp.str(R.string.err_archive_missing, "%1\$s: the archive had no %2\$s", LABEL, FILE),
            )
        }
        if (tmp.length() != FILE_BYTES) {
            ...
            throw IOException(
                NmApp.str(
                    R.string.err_unpack_size,
                    "%1\$s: unpacked %2\$d bytes, expected %3\$d",
                    LABEL, n, FILE_BYTES,
                ),
            )
        }
        ...
        if (!tmp.renameTo(file(context))) {
            throw IOException(
                NmApp.str(
                    R.string.err_cannot_replace,
                    "%1\$s: cannot replace %2\$s",
                    LABEL, file(context).toString(),
                ),
            )
        }
        zip.delete()
