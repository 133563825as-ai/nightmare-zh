...
data class Recipe(
    ...
    /**
     * ⭐⭐ **The card's text, translated.**
     *
     * ⚠⚠ **Why this is an accessor and not a change to [label] / [about].**
     * Those two are `val`s on a `data class` built into the top-level
     * `RECIPES` list, which is initialised while the class loader is still
     * running — before `Application.onCreate`, so before [com.abrah.nightmare.NmApp]
     * holds a Context. Reading a resource there would return the English
     * fallback and CACHE it for the process lifetime, i.e. a translation that
     * never applies.
     *
     * ⚠ So the literals stay as they are (they are also the fallback a unit
     * test and any untranslated locale sees) and the SCREEN asks for these
     * instead. One accessor, so the five places that draw a recipe cannot
     * drift apart.
     *
     * ⚠ `when` on [id], not on the label: two recipes must never share an id,
     * and an id is what the flow file on disk records — a renamed English
     * label cannot silently change the key.
     */
    /** ⭐ The card's title, in the phone's language where one exists. */
    val labelText: String
        get() {
            val res = when (id) {
                "txt2img" -> com.abrah.nightmare.R.string.recipe_txt2img_label
                "img2img" -> com.abrah.nightmare.R.string.recipe_img2img_label
                "inpaint" -> com.abrah.nightmare.R.string.recipe_inpaint_label
                "upscale" -> com.abrah.nightmare.R.string.recipe_upscale_label
                "flux_edit" -> com.abrah.nightmare.R.string.recipe_flux_edit_label
                "t2v" -> com.abrah.nightmare.R.string.recipe_t2v_label
                "i2v" -> com.abrah.nightmare.R.string.recipe_i2v_label
                else -> return label
            }
            return com.abrah.nightmare.NmApp.str(res, label)
        }

    /** ⭐ The card's description, in the phone's language where one exists. */
    val aboutText: String
        get() {
            val res = when (id) {
                "txt2img" -> com.abrah.nightmare.R.string.recipe_txt2img_about
                "img2img" -> com.abrah.nightmare.R.string.recipe_img2img_about
                "inpaint" -> com.abrah.nightmare.R.string.recipe_inpaint_about
                "upscale" -> com.abrah.nightmare.R.string.recipe_upscale_about
                "flux_edit" -> com.abrah.nightmare.R.string.recipe_flux_edit_about
                "t2v" -> com.abrah.nightmare.R.string.recipe_t2v_about
                "i2v" -> com.abrah.nightmare.R.string.recipe_i2v_about
                else -> return about
            }
            return com.abrah.nightmare.NmApp.str(res, about)
        }
