...
fun PictureActions(
    ...
    onDelete?.let {
        IconButton(onClick = { confirming = true }) {
            Icon(Icons.Filled.Delete, contentDescription = stringResource(R.string.cd_clear_what, what), tint = deleteTint)
        }
    }
        ...
        IconButton(
            ...
            Icon(
                ...
                contentDescription = when {
                    ...
                    kept -> stringResource(R.string.cd_kept_in_results)
                    else -> stringResource(R.string.cd_keep_what, what)
                },
    ...
    onDownload?.let { download ->
        IconButton(
            ...
            Icon(
                ...
                contentDescription = stringResource(R.string.cd_save_what, what),
                tint = tint,
            )
        }
    }
    onShare?.let { share ->
        IconButton(onClick = share) {
            Icon(com.abrah.nightmare.ui.ShareIcon, contentDescription = stringResource(R.string.cd_share_what, what), tint = tint)
        }
    }
    ...
    if (confirming && onDelete != null) {
        ConfirmDelete(
            title = stringResource(R.string.clear_what_title, what),
            confirmLabel = stringResource(R.string.clear_confirm),
            // ⚠ Says whether there is another copy — the render is often the only one.
            body = stringResource(R.string.clear_what_body) + " " + when {
                downloaded -> stringResource(R.string.clear_saved_note)
                kept -> stringResource(R.string.clear_autosave_note)
                else -> stringResource(R.string.clear_unsaved_note)
            },
            onConfirm = onDelete,
