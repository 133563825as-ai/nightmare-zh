...
private fun migrateCropNodes(
    ...
    notes += "removed the crop node" + (if (crops.size == 1) " " else "s ") +
        crops.keys.joinToString(", ") { "\"$it\"" } +
        " — crop nodes no longer exist; the framing moved onto the node each fed"
...
class WorkflowStore(private val dir: File) {
    ...
    fun rename(from: String, to: String): String? {
        ...
        val src = file(from)
        if (!src.isFile) {
            return NmApp.str(R.string.err_flow_gone, "\"%1\$s\" is gone", from)
        }
        if (file(target).exists()) {
            return NmApp.str(R.string.err_flow_exists, "\"%1\$s\" already exists", target)
        }
    ...
    fun validName(name: String): String? {
        val n = name.trim()
        return when {
            n.isEmpty() -> NmApp.str(R.string.err_give_it_a_name, "give it a name")
