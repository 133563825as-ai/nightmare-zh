...
object CrashReport {
    ...
    private fun headlineFor(exit: ApplicationExitInfo): String {
        val what = when (exit.reason) {
            ApplicationExitInfo.REASON_CRASH -> NmApp.str(R.string.crash_what_crash, "the app crashed")
            ApplicationExitInfo.REASON_CRASH_NATIVE ->
                NmApp.str(R.string.crash_what_crash_native, "the app crashed in native code")
            ApplicationExitInfo.REASON_ANR ->
                NmApp.str(R.string.crash_what_anr, "the app stopped responding")
            ApplicationExitInfo.REASON_LOW_MEMORY ->
                NmApp.str(R.string.crash_what_low_memory, "Android closed the app to free memory")
            ApplicationExitInfo.REASON_SIGNALED ->
                NmApp.str(R.string.crash_what_signaled, "Android stopped the app")
            else -> NmApp.str(R.string.crash_what_unknown, "the app closed unexpectedly")
        }
        val detail = exit.description?.trim()?.takeIf { it.isNotEmpty() }
        return if (detail != null) {
            NmApp.str(R.string.crash_headline_detail, "%1\$s — %2\$s", what, detail)
        } else what
    }
}
