...
object ModelInstaller {
    ...
    fun install(
        ...
        if (!inFlight.add(spec.id)) {
            throw IOException(
                NmApp.str(
                    R.string.err_already_downloading,
                    "%1\$s is already downloading",
                    spec.label,
                ),
            )
        }
    ...
    private fun installOnce(
        ...
        if (spec.files.isNotEmpty()) {
            ...
            if (missing.isNotEmpty()) {
                throw IOException(
                    NmApp.str(
                        R.string.err_install_incomplete,
                        "install incomplete, still missing: %1\$s",
    ...
    fun delete(context: Context, spec: ModelSpec, force: Boolean = false) {
        if (!force && spec.id == SelectedModel.id) {
            throw IllegalStateException(
                NmApp.str(
                    R.string.err_is_selected_model,
                    "\"%1\$s\" is the selected model; pick another first",
                    spec.label,
                ),
            )
        }
    ...
    private fun requireFreeSpace(dir: File, needed: Long) {
        ...
        if (free < needed) {
            throw IOException(
                NmApp.str(
                    R.string.err_not_enough_space,
                    "not enough free space: need ~%1\$d MB, have %2\$d MB",
                    needed shr 20,
                    free shr 20,
                )
            )
